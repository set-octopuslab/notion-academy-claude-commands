...
Claude Code의 노션 MCP가 활성화되어 있어야 명령어들이 노션 API 호출 가능합니다. 받는 분이 가이드 2-4 단계에서 미리 활성화하셨어야 합니다.

### Claude가 점검

`mcp__notionApi__API-get-self` 같은 가벼운 API 호출 시도. 응답이 정상이면 활성화됨.

### 활성화되지 않은 경우 (⚠️ PowerShell·터미널·파일 편집 안내 절대 금지)

받는 분에게 정확히 이 톤으로 안내하세요:

```
노션 MCP가 아직 활성화되지 않았어요. 셋업을 잠깐 멈춥니다.

가이드 HTML의 **2-4. Claude Code에 노션 MCP 활성화 + 연동키 등록** 단계를 먼저 끝내주세요.
Claude Code 설정 메뉴에서 클릭 몇 번이면 됩니다 — PowerShell이나 파일 편집은 필요 없어요.

활성화한 다음 Claude Code를 재시작하고, 새 세션에서 셋업 자연어 한 줄을 다시 붙여넣으면
1단계부터 자동으로 다시 시작합니다.
```

**핵심 원칙 (Claude에게)**:

- 받는 분이 PowerShell·터미널·파일 편집을 만지지 않게 하는 게 SETUP_FLOW의 절대 원칙입니다.
- 받는 분이 "어떻게 활성화해요?" 라고 물으면 가이드 2-4 단계로 안내하고, GUI 메뉴 클릭 방법을 안내하세요.
- `claude_desktop_config.json`, `~/.claude.json`, `mcp.json` 같은 파일을 PowerShell·터미널로 편집하라고 절대 안내하지 마세요. GUI 메뉴로 못 찾으면 "Claude Code 공식 안내 참고 또는 본인 환경 자세히 알려주고 다시 물어보세요" 정도까지만.
- 활성화 안 된 상태에서 셋업을 계속 진행하려 하지 마세요. 6단계 이후는 노션 API 호출이 필요해서 활성화 없이는 무의미합니다.
